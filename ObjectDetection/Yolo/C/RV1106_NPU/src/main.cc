...
int main(int argc, char *argv[])
{
    ...
    double rknn_init_time_ms, total_time_ms, load_network_time_ms, load_image_time_ms, post_processing_time_ms,
        prediction_time_ms, detection_time_ms, conversion_time_ms, load_classes_time_ms;
    ;
    clock_gettime(CLOCK_MONOTONIC, &start_time_ms);
    if (argc < 4)
    {
        printf("Usage: %s model_path input_path class_path [loop_count]\n", argv[0]);
        printf("Where:\n");
        printf("  model_path: Path to the model file.\n");
        printf("  input_path: Path to the target image we wish to classify.\n");
        printf("  class_path: Path to the class labels file.\n");
        printf("  loop_count (optional): Number of times to run the classification.\n");
        return -1;
    }

    char *model_path = argv[1];
    char *input_path = argv[2];
    char *class_path = argv[3];
    ...
    // Debug prints to confirm argument parsing
    printf("Model Path: %s\n", model_path);
    printf("Input Path: %s\n", input_path);
    printf("Class Path: %s\n", class_path);
    printf("Loop Count: %d\n", loop_count);
    ...
    rknn_init_time_ms = get_time_difference_ms(start_time_ms, end_time_ms);
    ...
    */
    ...
    if (width == stride)
    {
        printf("Copying input data to input tensor memory: width =%d, stride=%d\n", width, stride);
        memcpy(input_mems[0]->virt_addr, input_data, width * input_attrs[0].dims[1] * input_attrs[0].dims[3]);
    }
    else
    {
        printf("Copying input data to input tensor memory after reformat\n");
    ...
    post_process((int8_t *)output_mems[0]->virt_addr, (int8_t *)output_mems[1]->virt_addr,
                 (int8_t *)output_mems[2]->virt_addr, 640, 640,
                 box_conf_threshold, nms_threshold, scale_w, scale_h, out_zps,
                 out_scales, &detect_result_group, class_path, &load_classes_time_ms);
    clock_gettime(CLOCK_MONOTONIC, &end_time_ms);
    ...
    total_time_ms = load_network_time_ms + load_image_time_ms + prediction_time_ms + detection_time_ms + post_processing_time_ms;
    printf("\nTiming statistics:\n");
