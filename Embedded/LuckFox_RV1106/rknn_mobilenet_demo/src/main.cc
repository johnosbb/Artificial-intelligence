...
// The NPU output of the quantized model is in the int8 data type, and the post-processing should handle the data as int8.
// Below is code that provides the conversion from the NC1HWC2 format in int8 to the NCHW format in float.
...
// The NPU output of the quantized model is in the fp16 data type, and the post-processing should handle the data as fp16.
// Below is code that provides the conversion from the NC1HWC2 format in fp16 to the NCHW format in float.
...
int main(int argc, char *argv[])
{
    if (argc < 3)
    {
        printf("Usage:%s model_path input_path (.i.e image path) [loop_count]\n", argv[0]);
        return -1;
    }
