...
function get_available_board()
{
    ...
    if [ $MILKV_BOARD_ARRAY_LEN -eq 0 ]; then
        echo "DEBUG: No available config"
        exit 1
    fi
...
{
    echo "DEBUG: Cleaning old images..."
    ...
    echo "DEBUG: Cleaning up previous build files..."
...
echo "DEBUG: Getting available boards..."
get_available_board
...
echo "DEBUG: Getting tool chain"
get_toolchain

export MILKV_BOARD="${MILKV_BOARD}"

echo "DEBUG: Selected board: ${MILKV_BOARD}"
echo "DEBUG: Board Config: ${MILKV_BOARD_CONFIG}"
